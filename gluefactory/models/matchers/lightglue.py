...
def sigmoid_log_double_softmax(
    ...
    """根据逻辑和相似度创建log assignment matrix"""
    b, m, n = sim.shape
...
class LightGlue(nn.Module):
    default_conf = {
        ...
        "loss": {
            "gamma": 1.0,
            "fn": "nll",
            "nll_balancing": 0.5, # 正例的权重, 反例的权重为1 - nll_balancing
        },
    }
    ...
    def forward(self, data: dict) -> dict:
        ...
        if self.conf.add_scale_ori:
            # data["ori0/1"]是original_image_size
        ...
        for i in range(self.conf.n_layers):
            ...
            if do_early_stop:
                ...
                # TODO: 修改early stop的判断条件, 改为可学习两幅图像的旋转矩阵和平移向量的误差的值
    ...
    def loss(self, pred, data):
        ...
        for i in range(N - 1):
            ...
            # 计算confidence loss
            # pred['ref_descriptors0'][:, i] 是第i层的描述子, 内含batch_size个描述子, 每个描述子代表一个特征点, 256维
            # params_i['log_assignment'] 是第i层的log assignment, 内含batch_size个log assignment, 每个log assignment代表一个特征点, 257维
            # pred['log_assignment'] 是最后一层的log assignment, 内含batch_size个log assignment, 每个log assignment代表一个特征点, 257维
