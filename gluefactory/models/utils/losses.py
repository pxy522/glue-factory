...
def weight_loss(log_assignment, weights, gamma=0.0):
    ...
    loss_sc = log_assignment * weights                          # B x (M+1) x (N+1)

    num_neg0 = weights[:, :m, -1].sum(-1).clamp(min=1.0)        # B x 1
    num_neg1 = weights[:, -1, :n].sum(-1).clamp(min=1.0)        # B x 1
    num_pos = weights[:, :m, :n].sum((-1, -2)).clamp(min=1.0)   # B x 1
